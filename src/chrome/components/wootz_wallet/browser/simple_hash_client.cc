...
#include "chrome/components/wootz_wallet/browser/json_rpc_response_parser.h"
...
#include "chrome/components/wootz_wallet/common/string_utils.h"
#include "chrome/components/constants/wootz_services_key.h"
#include "chrome/components/json/rs/src/lib.rs.h"
  ...
  return net::DefineNetworkTrafficAnnotation("chrome_wallet_service", R"(
      semantics {
        ...
        description:
          ...
          "of the user interacting with the native Wootz wallet."
        trigger:
          "Triggered by uses of the native Wootz wallet."
        data:
          "NFT assets."
        destination: WEBSITE
      }
...
constexpr char kSimpleHashCdnWootzProxyHost[] =
    "simplehash.wallet-cdn.wootz.com";

std::optional<std::string> ChainIdToSimpleHashChainId(
    const std::string& chain_id) {
  static base::NoDestructor<base::flat_map<std::string, std::string>>
      chain_id_lookup({
          {wootz_wallet::mojom::kMainnetChainId, kEthereum},
          {wootz_wallet::mojom::kSolanaMainnet, kSolana},
          {wootz_wallet::mojom::kPolygonMainnetChainId, kPolygon},
          {wootz_wallet::mojom::kArbitrumMainnetChainId, kArbitrum},
          {wootz_wallet::mojom::kOptimismMainnetChainId, kOptimism},
          {wootz_wallet::mojom::kAvalancheMainnetChainId, kAvalanche},
          {wootz_wallet::mojom::kBnbSmartChainMainnetChainId, kBsc},
          ...
          {wootz_wallet::mojom::kPalmChainId, kPalm},
          {wootz_wallet::mojom::kPolygonZKEVMChainId, kPolygonZkEvm},
          {wootz_wallet::mojom::kZkSyncEraChainId, kZkSyncEra},
      });
...
std::optional<std::string> SimpleHashChainIdToChainId(
  ...
  static base::NoDestructor<base::flat_map<std::string, std::string>>
      simple_hash_chain_id_lookup({
          {kEthereum, wootz_wallet::mojom::kMainnetChainId},
          {kSolana, wootz_wallet::mojom::kSolanaMainnet},
          {kPolygon, wootz_wallet::mojom::kPolygonMainnetChainId},
          {kArbitrum, wootz_wallet::mojom::kArbitrumMainnetChainId},
          {kOptimism, wootz_wallet::mojom::kOptimismMainnetChainId},
          {kAvalanche, wootz_wallet::mojom::kAvalancheMainnetChainId},
          {kBsc, wootz_wallet::mojom::kBnbSmartChainMainnetChainId},
          ...
          {kPalm, wootz_wallet::mojom::kPalmChainId},
          {kPolygonZkEvm, wootz_wallet::mojom::kPolygonZKEVMChainId},
          {kZkSyncEra, wootz_wallet::mojom::kZkSyncEraChainId},
      });
...
// Calls
// https://simplehash.wallet.wootz.com/api/v0/nfts/owners?chains={chains}&wallet_addresses={wallet_addresses}
...
// Calls
// https://simplehash.wallet.wootz.com/api/v0/nfts/proof/solana/{token_address}
void SimpleHashClient::FetchSolCompressedNftProofData(
  ...
  GURL url = GURL(base::StrCat(
      {kSimpleHashWootzProxyUrl, "/api/v0/nfts/proof/solana/", token_address}));
  if (!url.is_valid()) {
    std::move(callback).Run(std::nullopt);
    return;
  }
...
void SimpleHashClient::OnGetNfts(
  ...
  if (nft_identifiers.size() > 0) {
    ...
    api_request_helper_->Request(
        ...
        MakeWootzServicesKeyHeaders(), {.auto_retry_on_network_change = true});
    return;
  }
  ...
  for (const auto& nft_value : *nfts) {
    ...
    if (coin == mojom::CoinType::ETH) {
      auto checksum_address =
          wootz_wallet::EthAddress::ToEip1191ChecksumAddress(*contract_address,
                                                             *chain_id_str);
      if (!checksum_address) {
        continue;
      }
...
SimpleHashClient::ParseMetadatas(const base::Value& json_value,
  ...
  for (const auto& nft_value : *nfts) {
    ...
    if (image) {
      ...
      replacements.SetHostStr(kSimpleHashCdnWootzProxyHost);
...
GURL SimpleHashClient::GetSimpleHashNftsByWalletUrl(
  ...
  std::string urlStr =
      base::StrCat({kSimpleHashWootzProxyUrl, "/api/v0/nfts/owners"});
...
GURL SimpleHashClient::GetNftsUrl(
  ...
  GURL url =
      GURL(base::StrCat({kSimpleHashWootzProxyUrl, "/api/v0/nfts/assets"}));
