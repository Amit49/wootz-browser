...
TokenListMap BlockchainRegistry::GetEthTokenListMap(
  ...
  for (const auto& chain_id : chain_ids) {
    ...
    const auto& tokens = token_list_map_[key];
    std::vector<wootz_wallet::mojom::BlockchainTokenPtr> tokens_copy(
        tokens.size());
    std::transform(
        tokens.begin(), tokens.end(), tokens_copy.begin(),
        [](const wootz_wallet::mojom::BlockchainTokenPtr& current_token)
            -> wootz_wallet::mojom::BlockchainTokenPtr {
