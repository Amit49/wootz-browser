...
#include "components/wootz_wallet/renderer/resource_helper.h"
#include "components/wootz_wallet/renderer/v8_helper.h"
#include "components/wootz_wallet/resources/grit/wootz_wallet_script_generated.h"
...
// #include "third_party/blink/public/platform/browser_interface_broker_proxy.h"  // New location
#include "third_party/blink/public/common/browser_interface_broker_proxy.h" // Old Chromium file
...
constexpr char kWootzEthereum[] = "wootzEthereum";
constexpr char kEthereum[] = "ethereum";
constexpr char kEmit[] = "emit";
constexpr char kIsWootzWallet[] = "isWootzWallet";
...
void JSEthereumProvider::Install(bool install_ethereum_provider,
  ...
  // Check window.wootzEthereum existence.
  ...
  // Set window.wootzEthereum
  SetProviderNonWritable(context, global, ethereum_proxy,
                         gin::StringToV8(isolate, kWootzEthereum), true);
...
void JSEthereumProvider::FireEvent(const std::string& event,
  ...
  CallMethodOfObject(render_frame()->GetWebFrame(), kWootzEthereum, kEmit,
                     std::move(args));
}
...
void JSEthereumProvider::AnnounceProvider() {
  ...
  auto provider =
      GetProperty(context, context->Global(), kWootzEthereum).ToLocalChecked();
...
const std::string& JSEthereumProvider::GetWootzWalletImage() {
  if (!wootz_wallet_image_) {
    wootz_wallet_image_ =
        LoadImageResourceAsDataUrl(IDR_WOOTZ_WALLET_PROVIDER_ICON);
  }
  return wootz_wallet_image_.value();
}
