...
constexpr char kWootzSolana[] = "wootzSolana";
...
bool JSSolanaProvider::GetIsWootzWallet(gin::Arguments* arguments) {
  return true;
}
...
void JSSolanaProvider::FireEvent(
  ...
  CallMethodOfObject(render_frame()->GetWebFrame(), kWootzSolana, kEmit,
                     std::move(args));
}
...
bool JSSolanaProvider::LoadSolanaWeb3ModuleIfNeeded(v8::Isolate* isolate) {
  ...
  std::string solana_web3_module_str = base::StrCat(
      {"(function() {", LoadDataResource(IDR_WOOTZ_WALLET_SOLANA_WEB3_JS),
       "return solanaWeb3; })()"});
