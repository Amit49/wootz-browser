...
#include "components/wootz_wallet/browser/eth_allowance_manager.h"
...
#include "components/wootz_wallet/common/fil_address.h"
...
WootzWalletService::WootzWalletService(
    ...
    std::unique_ptr<WootzWalletServiceDelegate> delegate,
  ...
  wootz_wallet_p3a_ = std::make_unique<WootzWalletP3A>(
  ...
  pref_change_registrar_.Add(
      ...
      base::BindRepeating(&WootzWalletService::OnWalletUnlockPreferenceChanged,
                          base::Unretained(this)));
  pref_change_registrar_.Add(
      kDefaultEthereumWallet,
      base::BindRepeating(&WootzWalletService::OnDefaultEthereumWalletChanged,
                          base::Unretained(this)));
  pref_change_registrar_.Add(
      kDefaultSolanaWallet,
      base::BindRepeating(&WootzWalletService::OnDefaultSolanaWalletChanged,
                          base::Unretained(this)));
  pref_change_registrar_.Add(
      kDefaultBaseCurrency,
      base::BindRepeating(&WootzWalletService::OnDefaultBaseCurrencyChanged,
                          base::Unretained(this)));
  pref_change_registrar_.Add(
      kDefaultBaseCryptocurrency,
      base::BindRepeating(
          &WootzWalletService::OnDefaultBaseCryptocurrencyChanged,
          base::Unretained(this)));
  pref_change_registrar_.Add(
      kWootzWalletCustomNetworks,
      base::BindRepeating(&WootzWalletService::OnNetworkListChanged,
                          base::Unretained(this)));
  pref_change_registrar_.Add(
      kWootzWalletHiddenNetworks,
      base::BindRepeating(&WootzWalletService::OnNetworkListChanged,
                          base::Unretained(this)));
  pref_change_registrar_.Add(
      kWootzWalletNftDiscoveryEnabled,
      base::BindRepeating(&WootzWalletService::OnWootzWalletNftDiscoveryEnabled,
                          base::Unretained(this)));
  pref_change_registrar_.Add(
      kWootzWalletSelectedNetworks,
      base::BindRepeating(&WootzWalletService::OnNetworkChanged,
                          weak_ptr_factory_.GetWeakPtr()));

  // Added 05/2024 to label compressed nfts as such.
  WootzWalletService::MaybeMigrateCompressedNfts();
}

WootzWalletService::WootzWalletService() : weak_ptr_factory_(this) {}

WootzWalletService::~WootzWalletService() = default;

// For unit tests
void WootzWalletService::RemovePrefListenersForTests() {
  pref_change_registrar_.RemoveAll();
}

template <>
void WootzWalletService::Bind(
    mojo::PendingReceiver<mojom::WootzWalletService> receiver) {
...
void WootzWalletService::Bind(
  ...
  GetWootzWalletP3A()->Bind(std::move(receiver));
}

void WootzWalletService::GetUserAssets(const std::string& chain_id,
                                       mojom::CoinType coin,
                                       GetUserAssetsCallback callback) {
  std::vector<mojom::BlockchainTokenPtr> result;
  for (auto& item : ::wootz_wallet::GetAllUserAssets(profile_prefs_)) {
...
void WootzWalletService::GetAllUserAssets(GetUserAssetsCallback callback) {
  std::vector<mojom::BlockchainTokenPtr> result =
      ::wootz_wallet::GetAllUserAssets(profile_prefs_);
  std::move(callback).Run(std::move(result));
}

bool WootzWalletService::AddUserAssetInternal(mojom::BlockchainTokenPtr token) {
  if (!network_manager()->GetChain(token->chain_id, token->coin)) {
    return false;
  }

  auto added_asset =
      ::wootz_wallet::AddUserAsset(profile_prefs_, std::move(token));
  if (!added_asset) {
    return false;
  }
...
void WootzWalletService::AddUserAsset(mojom::BlockchainTokenPtr token,
  ...
  if (token->is_nft && token->coin == mojom::CoinType::ETH) {
    ...
    json_rpc_service_->GetEthNftStandard(
        ...
        base::BindOnce(&WootzWalletService::OnGetEthNftStandard,
    ...
    simple_hash_client_->GetNfts(
        ...
        base::BindOnce(&WootzWalletService::OnGetNfts,
...
void WootzWalletService::OnGetEthNftStandard(
...
void WootzWalletService::OnGetNfts(
    AddUserAssetCallback callback,
...
void WootzWalletService::RemoveUserAsset(mojom::BlockchainTokenPtr token,
                                         RemoveUserAssetCallback callback) {
  std::move(callback).Run(RemoveUserAsset(std::move(token)));
}

bool WootzWalletService::RemoveUserAsset(mojom::BlockchainTokenPtr token) {
  if (!::wootz_wallet::RemoveUserAsset(profile_prefs_, token)) {
    return false;
  }
...
void WootzWalletService::SetUserAssetVisible(
    mojom::BlockchainTokenPtr token,
    bool visible,
...
bool WootzWalletService::SetUserAssetVisible(mojom::BlockchainTokenPtr token,
                                             bool visible) {
  CHECK(token);

  return ::wootz_wallet::SetUserAssetVisible(profile_prefs_, token, visible);
}

void WootzWalletService::SetAssetSpamStatus(
    mojom::BlockchainTokenPtr token,
    bool is_spam,
...
bool WootzWalletService::SetAssetSpamStatus(mojom::BlockchainTokenPtr token,
                                            bool is_spam) {
  CHECK(token);

  if (::wootz_wallet::SetAssetSpamStatus(profile_prefs_, token, is_spam)) {
    return true;
  }
...
void WootzWalletService::IsExternalWalletInstalled(
...
void WootzWalletService::ImportFromExternalWallet(
  ...
  delegate_->GetImportInfoFromExternalWallet(
      type, password,
      base::BindOnce(&WootzWalletService::OnGetImportInfo,
                     weak_ptr_factory_.GetWeakPtr(), new_password,
                     std::move(callback)));
}

void WootzWalletService::GetDefaultEthereumWallet(
    GetDefaultEthereumWalletCallback callback) {
  std::move(callback).Run(
      ::wootz_wallet::GetDefaultEthereumWallet(profile_prefs_));
}

void WootzWalletService::GetDefaultSolanaWallet(
    GetDefaultSolanaWalletCallback callback) {
  std::move(callback).Run(
      ::wootz_wallet::GetDefaultSolanaWallet(profile_prefs_));
}

void WootzWalletService::SetDefaultEthereumWallet(
    mojom::DefaultWallet default_wallet) {
  auto old_default_wallet =
      ::wootz_wallet::GetDefaultEthereumWallet(profile_prefs_);
  if (old_default_wallet != default_wallet) {
    ::wootz_wallet::SetDefaultEthereumWallet(profile_prefs_, default_wallet);
  }
}

void WootzWalletService::SetDefaultSolanaWallet(
    mojom::DefaultWallet default_wallet) {
  auto old_default_wallet =
      ::wootz_wallet::GetDefaultSolanaWallet(profile_prefs_);
  if (old_default_wallet != default_wallet) {
    ::wootz_wallet::SetDefaultSolanaWallet(profile_prefs_, default_wallet);
  }
}

void WootzWalletService::GetDefaultBaseCurrency(
    GetDefaultBaseCurrencyCallback callback) {
  std::move(callback).Run(
      ::wootz_wallet::GetDefaultBaseCurrency(profile_prefs_));
}

void WootzWalletService::SetDefaultBaseCurrency(const std::string& currency) {
  auto old_default_currency =
      ::wootz_wallet::GetDefaultBaseCurrency(profile_prefs_);
  if (old_default_currency != currency) {
    ::wootz_wallet::SetDefaultBaseCurrency(profile_prefs_, currency);
  }
}

void WootzWalletService::GetDefaultBaseCryptocurrency(
    GetDefaultBaseCryptocurrencyCallback callback) {
  std::move(callback).Run(
      ::wootz_wallet::GetDefaultBaseCryptocurrency(profile_prefs_));
}

void WootzWalletService::SetDefaultBaseCryptocurrency(
    const std::string& cryptocurrency) {
  auto old_default_cryptocurrency =
      ::wootz_wallet::GetDefaultBaseCryptocurrency(profile_prefs_);
  if (old_default_cryptocurrency != cryptocurrency) {
    ::wootz_wallet::SetDefaultBaseCryptocurrency(profile_prefs_,
                                                 cryptocurrency);
  }
}

void WootzWalletService::EnsureSelectedAccountForChain(
    mojom::CoinType coin,
    const std::string& chain_id,
...
mojom::AccountIdPtr WootzWalletService::EnsureSelectedAccountForChainSync(
    mojom::CoinType coin,
    const std::string& chain_id) {
...
void WootzWalletService::GetNetworkForSelectedAccountOnActiveOrigin(
...
WootzWalletService::GetNetworkForSelectedAccountOnActiveOriginSync() {
...
void WootzWalletService::SetNetworkForSelectedAccountOnActiveOrigin(
    const std::string& chain_id,
...
void WootzWalletService::OnDefaultSolanaWalletChanged() {
  auto default_wallet = ::wootz_wallet::GetDefaultSolanaWallet(profile_prefs_);
  for (const auto& observer : observers_) {
    observer->OnDefaultSolanaWalletChanged(default_wallet);
  }
}

void WootzWalletService::OnDefaultBaseCurrencyChanged() {
  auto value = ::wootz_wallet::GetDefaultBaseCurrency(profile_prefs_);
  for (const auto& observer : observers_) {
    observer->OnDefaultBaseCurrencyChanged(value);
  }
}

void WootzWalletService::OnDefaultBaseCryptocurrencyChanged() {
  auto value = ::wootz_wallet::GetDefaultBaseCryptocurrency(profile_prefs_);
  for (const auto& observer : observers_) {
    observer->OnDefaultBaseCryptocurrencyChanged(value);
  }
}

void WootzWalletService::OnNetworkListChanged() {
  for (const auto& observer : observers_) {
    observer->OnNetworkListChanged();
  }
}

void WootzWalletService::OnWootzWalletNftDiscoveryEnabled() {
  if (profile_prefs_->GetBoolean(kWootzWalletNftDiscoveryEnabled)) {
    DiscoverAssetsOnAllSupportedChains(true);
  }
}

void WootzWalletService::HasPermission(
...
void WootzWalletService::ResetPermission(mojom::AccountIdPtr account_id,
...
void WootzWalletService::IsPermissionDenied(
    mojom::CoinType coin,
...
// static
void WootzWalletService::MigrateHiddenNetworks(PrefService* prefs) {
  auto previous_version_code =
      prefs->GetInteger(kWootzWalletDefaultHiddenNetworksVersion);
  if (previous_version_code >= 1) {
    return;
  }
  {
    // Default hidden networks
    ScopedDictPrefUpdate update(prefs, kWootzWalletHiddenNetworks);
  ...
  prefs->SetInteger(kWootzWalletDefaultHiddenNetworksVersion, 1);
}
...
bool ShouldMigrateRemovedPreloadedNetwork(PrefService* prefs,
  ...
  const auto& selected_networks =
      prefs->GetDict(kWootzWalletSelectedNetworksPerOrigin);
  ...
  const auto& selected_default_networks =
      prefs->GetDict(kWootzWalletSelectedNetworks);
...
void WootzWalletService::MigrateFantomMainnetAsCustomNetwork(
    PrefService* prefs) {
  if (prefs->GetBoolean(kWootzWalletCustomNetworksFantomMainnetMigrated)) {
    return;
  }
  ...
  prefs->SetBoolean(kWootzWalletCustomNetworksFantomMainnetMigrated, true);
}

void WootzWalletService::MigrateAssetsPrefToList(PrefService* prefs) {
  if (!prefs->HasPrefPath(kWootzWalletUserAssetsDeprecated)) {
    return;
  }
  ...
  const auto& user_assets_dict =
      prefs->GetDict(kWootzWalletUserAssetsDeprecated);
  ...
  prefs->SetList(kWootzWalletUserAssetsList, std::move(assets_list));

  prefs->ClearPref(kWootzWalletUserAssetsDeprecated);
}

// static
void WootzWalletService::MigrateEip1559ForCustomNetworks(PrefService* prefs) {
  if (prefs->GetBoolean(kWootzWalletEip1559ForCustomNetworksMigrated)) {
    return;
  }
  prefs->SetBoolean(kWootzWalletEip1559ForCustomNetworksMigrated, true);
  ...
  ScopedDictPrefUpdate update(prefs, kWootzWalletCustomNetworks);
...
void WootzWalletService::MaybeMigrateCompressedNfts() {
  if (profile_prefs_->GetBoolean(kWootzWalletIsCompressedNftMigrated)) {
    return;
  }
  ...
  simple_hash_client_->GetNfts(
      mojom::CoinType::SOL, std::move(nft_ids),
      base::BindOnce(&WootzWalletService::OnGetNftsForCompressedMigration,
                     weak_ptr_factory_.GetWeakPtr()));
}

void WootzWalletService::OnGetNftsForCompressedMigration(
    std::vector<mojom::BlockchainTokenPtr> nfts) {
  for (auto& nft : nfts) {
    ...
    if (!::wootz_wallet::SetAssetCompressed(profile_prefs_, nft)) {
      continue;
    }
  }

  profile_prefs_->SetBoolean(kWootzWalletIsCompressedNftMigrated, true);
}

void WootzWalletService::OnWalletUnlockPreferenceChanged(
    const std::string& pref_name) {
  wootz_wallet_p3a_->ReportUsage(true);
}

WootzWalletP3A* WootzWalletService::GetWootzWalletP3A() {
  return wootz_wallet_p3a_.get();
}

BitcoinWalletService* WootzWalletService::GetBitcoinWalletService() {
  return bitcoin_wallet_service_.get();
}

ZCashWalletService* WootzWalletService::GetZcashWalletService() {
  return zcash_wallet_service_.get();
}

void WootzWalletService::GetActiveOrigin(GetActiveOriginCallback callback) {
  std::move(callback).Run(GetActiveOriginSync());
}

mojom::OriginInfoPtr WootzWalletService::GetActiveOriginSync() {
...
void WootzWalletService::NotifySignMessageRequestProcessed(
    bool approved,
    int id,
...
void WootzWalletService::NotifySignMessageErrorProcessed(
    const std::string& id) {
...
void WootzWalletService::GetPendingSignTransactionRequests(
...
void WootzWalletService::NotifySignTransactionRequestProcessed(
    bool approved,
    int id,
    mojom::ByteArrayStringUnionPtr signature,
    const std::optional<std::string>& error) {
...
void WootzWalletService::GetPendingSignAllTransactionsRequests(
...
void WootzWalletService::NotifySignAllTransactionsRequestProcessed(
    bool approved,
    int id,
...
void WootzWalletService::AddObserver(
    ::mojo::PendingRemote<mojom::WootzWalletServiceObserver> observer) {
  observers_.Add(std::move(observer));
}

void WootzWalletService::AddTokenObserver(
    ::mojo::PendingRemote<mojom::WootzWalletServiceTokenObserver> observer) {
  token_observers_.Add(std::move(observer));
}

void WootzWalletService::OnActiveOriginChanged(
...
void WootzWalletService::WalletRestored() {
...
void WootzWalletService::OnDiscoverAssetsStarted() {
  for (const auto& observer : observers_) {
    observer->OnDiscoverAssetsStarted();
  }
}

void WootzWalletService::OnDiscoverAssetsCompleted(
...
void WootzWalletService::OnGetImportInfo(
    const std::string& new_password,
    ...
    switch (error) {
      case ImportError::kJsonError:
        std::move(callback).Run(false, l10n_util::GetStringUTF8(
                                           IDS_WOOTZ_WALLET_IMPORT_JSON_ERROR));
        break;
      case ImportError::kPasswordError:
        std::move(callback).Run(
            false,
            l10n_util::GetStringUTF8(IDS_WOOTZ_WALLET_IMPORT_PASSWORD_ERROR));
        break;
      case ImportError::kInternalError:
        std::move(callback).Run(
            false,
            l10n_util::GetStringUTF8(IDS_WOOTZ_WALLET_IMPORT_INTERNAL_ERROR));
        break;
...
void WootzWalletService::AddSignTransactionRequest(
...
void WootzWalletService::AddSignAllTransactionsRequest(
...
void WootzWalletService::AddSuggestTokenRequest(
  ...
  mojom::BlockchainTokenPtr token;
  for (auto& user_asset : ::wootz_wallet::GetAllUserAssets(profile_prefs_)) {
...
void WootzWalletService::AddGetPublicKeyRequest(
...
void WootzWalletService::AddDecryptRequest(
    const mojom::AccountIdPtr& account_id,
    const url::Origin& origin,
...
void WootzWalletService::GetPendingGetEncryptionPublicKeyRequests(
...
void WootzWalletService::GetPendingAddSuggestTokenRequests(
...
void WootzWalletService::GetPendingDecryptRequests(
...
void WootzWalletService::NotifyAddSuggestTokenRequestsProcessed(
    bool approved,
...
void WootzWalletService::NotifyGetPublicKeyRequestProcessed(
...
void WootzWalletService::NotifyDecryptRequestProcessed(
    const std::string& request_id,
    bool approved) {
...
void WootzWalletService::IsBase58EncodedSolanaPubkey(
    const std::string& key,
    IsBase58EncodedSolanaPubkeyCallback callback) {
  std::move(callback).Run(::wootz_wallet::IsBase58EncodedSolanaPubkey(key));
}

void WootzWalletService::Base58Encode(
  ...
  for (const auto& address : addresses) {
    encoded_addresses.push_back(wootz_wallet::Base58Encode(address));
  }
  std::move(callback).Run(std::move(encoded_addresses));
}

void WootzWalletService::DiscoverAssetsOnAllSupportedChains(
...
void WootzWalletService::GetNftDiscoveryEnabled(
    GetNftDiscoveryEnabledCallback callback) {
  std::move(callback).Run(
      profile_prefs_->GetBoolean(kWootzWalletNftDiscoveryEnabled));
}

void WootzWalletService::SetNftDiscoveryEnabled(bool enabled) {
  profile_prefs_->SetBoolean(kWootzWalletNftDiscoveryEnabled, enabled);
}

void WootzWalletService::GetPrivateWindowsEnabled(
    GetPrivateWindowsEnabledCallback callback) {
  std::move(callback).Run(
      profile_prefs_->GetBoolean(kWootzWalletPrivateWindowsEnabled));
}

void WootzWalletService::SetPrivateWindowsEnabled(bool enabled) {
  profile_prefs_->SetBoolean(kWootzWalletPrivateWindowsEnabled, enabled);
}

void WootzWalletService::GetBalanceScannerSupportedChains(
...
void WootzWalletService::ConvertFEVMToFVMAddress(
    bool is_mainnet,
...
void WootzWalletService::GenerateReceiveAddress(
  ...
  if (account_id->coin == mojom::CoinType::BTC) {
    ...
    bitcoin_wallet_service_->RunDiscovery(
        ...
        base::BindOnce(&WootzWalletService::OnGenerateBtcReceiveAddress,
                       weak_ptr_factory_.GetWeakPtr(), std::move(callback)));
    return;
  }
  ...
  if (account_id->coin == mojom::CoinType::ZEC) {
    ...
    zcash_wallet_service_->DiscoverNextUnusedAddress(
        std::move(account_id), false,
        base::BindOnce(&WootzWalletService::OnGenerateZecReceiveAddress,
                       weak_ptr_factory_.GetWeakPtr(), std::move(callback)));
    return;
  }
...
void WootzWalletService::OnGenerateBtcReceiveAddress(
    GenerateReceiveAddressCallback callback,
...
void WootzWalletService::OnGenerateZecReceiveAddress(
    GenerateReceiveAddressCallback callback,
...
void WootzWalletService::GetSimpleHashSpamNFTs(
...
void WootzWalletService::CancelAllSuggestedTokenCallbacks() {
...
void WootzWalletService::CancelAllSignMessageCallbacks() {
...
void WootzWalletService::CancelAllSignTransactionCallbacks() {
...
void WootzWalletService::CancelAllSignAllTransactionsCallbacks() {
...
void WootzWalletService::CancelAllGetEncryptionPublicKeyCallbacks() {
  base::Value formed_response = GetProviderErrorDictionary(
      mojom::ProviderError::kUserRejectedRequest,
      l10n_util::GetStringUTF8(IDS_WALLET_USER_REJECTED_REQUEST));
...
void WootzWalletService::CancelAllDecryptCallbacks() {
  base::Value formed_response = GetProviderErrorDictionary(
      mojom::ProviderError::kUserRejectedRequest,
      l10n_util::GetStringUTF8(IDS_WALLET_USER_REJECTED_REQUEST));
...
void WootzWalletService::OnNetworkChanged() {
  CancelAllSuggestedTokenCallbacks();
}

void WootzWalletService::Reset() {
  ...
  // Clear WootzWalletService
  ClearWootzWalletServicePrefs(profile_prefs_);
  CancelAllSuggestedTokenCallbacks();
...
void WootzWalletService::IsPrivateWindow(IsPrivateWindowCallback callback) {
  std::move(callback).Run(delegate_->IsPrivateWindow());
}

void WootzWalletService::GetTransactionSimulationOptInStatus(
    GetTransactionSimulationOptInStatusCallback callback) {
  std::move(callback).Run(
      ::wootz_wallet::GetTransactionSimulationOptInStatus(profile_prefs_));
}

void WootzWalletService::SetTransactionSimulationOptInStatus(
    mojom::BlowfishOptInStatus status) {
  ::wootz_wallet::SetTransactionSimulationOptInStatus(profile_prefs_, status);
}
