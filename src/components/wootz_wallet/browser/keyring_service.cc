...
#include "components/wootz_wallet/browser/bitcoin/bitcoin_hd_keyring.h"
#include "components/wootz_wallet/browser/bitcoin/bitcoin_import_keyring.h"
...
#include "components/wootz_wallet/common/switches.h"
...
std::string GetAccountName(size_t number) {
  return l10n_util::GetStringFUTF8(IDS_WOOTZ_WALLET_NUMBERED_ACCOUNT_NAME,
...
std::optional<std::string> DecryptWalletMnemonicFromPrefs(
  ...
  if (auto decrypted = encryptor.DecryptFromDict(
          profile_prefs->GetDict(kWootzWalletMnemonic))) {
...
std::vector<uint8_t> GetSaltFromPrefs(const PrefService* profile_prefs) {
  ...
  auto salt_encoded = profile_prefs->GetString(kWootzWalletEncryptorSalt);
...
bool IsLegacyEthSeedFormat(const PrefService* profile_prefs) {
  CHECK(profile_prefs);
  return profile_prefs->GetBoolean(kWootzWalletLegacyEthSeedFormat);
}
...
struct ImportedAccountInfo {
  ...
  ImportedAccountInfo(mojom::KeyringId keyring_id,
      ...
      : keyring_id(keyring_id),
        ...
        account_index(std::move(account_index)),
  ...
  base::Value ToValue() const {
    ...
    CHECK(account_address || account_index);
    if (account_address) {
  ...
  static std::optional<ImportedAccountInfo> FromValue(
    ...
    ImportedAccountInfo account_info(
        ...
        account_index, imported_private_key->Clone());
...
void SetImportedAccountsForKeyring(
  ...
  ScopedDictPrefUpdate update(profile_prefs, kWootzWalletKeyrings);
...
std::string GetSelectedDappAccountFromPrefs(PrefService* profile_prefs,
  ...
  const char* pref_name = dapp_coin == mojom::CoinType::ETH
                              ? kWootzWalletSelectedEthDappAccount
                              : kWootzWalletSelectedSolDappAccount;

  return profile_prefs->GetString(pref_name);
}

std::string GetSelectedWalletAccountFromPrefs(PrefService* profile_prefs) {
  return profile_prefs->GetString(kWootzWalletSelectedWalletAccount);
}
...
std::optional<KeyringSeed> MakeSeedFromMnemonic(
  ...
  if (use_legacy_eth_seed_format) {
    ...
    if (eth_seed->size() != 32) {
      VLOG(1) << __func__
              << "mnemonic for legacy wootz wallet must be 24 words which will "
...
bool KeyringService::IsWalletCreatedSync() {
  return profile_prefs_->GetDict(kWootzWalletKeyrings).size() > 0;
}
...
bool KeyringService::CreateWalletInternal(const std::string& mnemonic,
  ...
  profile_prefs_->SetBoolean(kWootzWalletKeyringEncryptionKeysMigrated, true);
  profile_prefs_->SetBoolean(kWootzWalletLegacyEthSeedFormat,
                             is_legacy_eth_seed_format);
  profile_prefs_->SetDict(
      kWootzWalletMnemonic,
      encryptor_->EncryptToDict(base::as_byte_span(mnemonic),
                                PasswordEncryptor::CreateNonce()));
  profile_prefs_->SetString(kWootzWalletEncryptorSalt,
...
void KeyringService::LoadAccountsFromPrefs(mojom::KeyringId keyring_id) {
  ...
  if (IsBitcoinImportKeyring(keyring_id)) {
    ...
    for (const auto& imported_account_info :
      ...
      CHECK(imported_account_info.account_index);
      keyring->AddAccount(
...
void KeyringService::ImportFilecoinAccount(
  ...
  ImportedAccountInfo imported_account_info(
      keyring_id, account_name, address, std::nullopt,
...
mojom::AccountInfoPtr KeyringService::ImportBitcoinAccountSync(
  ...
  uint32_t account_index = GetNextAccountIndex(profile_prefs_, keyring_id);

  if (!keyring->AddAccount(account_index, payload)) {
    return nullptr;
  }
  SetNextAccountIndex(profile_prefs_, keyring_id, account_index + 1);
...
bool KeyringService::IsWalletBackedUpSync() {
  return profile_prefs_->GetBoolean(kWootzWalletMnemonicBackedUp);
}

void KeyringService::NotifyWalletBackupComplete() {
  profile_prefs_->SetBoolean(kWootzWalletMnemonicBackedUp, true);
  for (const auto& observer : observers_) {
    observer->BackedUp();
  }
}
...
std::vector<mojom::AccountInfoPtr> KeyringService::AddHardwareAccountsSync(
  ...
  ScopedDictPrefUpdate keyrings_update(profile_prefs_, kWootzWalletKeyrings);
...
KeyringService::SignatureWithError KeyringService::SignMessageByDefaultKeyring(
    ...
    bool is_eip712) {
  CHECK(account_id);
  LOG(ERROR) << "JANGID KeyringService: SignMessageByDefaultKeyring started";
  LOG(ERROR) << "JANGID KeyringService: Account ID address: " << account_id->address;
  LOG(ERROR) << "JANGID KeyringService: Account ID keyring_id: " << account_id->keyring_id;
  LOG(ERROR) << "JANGID KeyringService: Account ID coin: " << static_cast<int>(account_id->coin);
  LOG(ERROR) << "JANGID KeyringService: Message length: " << message.size();
  LOG(ERROR) << "JANGID KeyringService: Message (hex): " << ToHex(message);
  LOG(ERROR) << "JANGID KeyringService: Is EIP712: " << (is_eip712 ? "Yes" : "No");

  SignatureWithError ret;
  auto* keyring = GetEthereumKeyring();
  LOG(ERROR) << "JANGID KeyringService: Keyring pointer: " << (keyring ? "Valid" : "Null");
  
  if (!keyring || account_id->keyring_id != mojom::kDefaultKeyringId) {
    ret.signature = std::nullopt;
    ret.error_message =
        l10n_util::GetStringUTF8(IDS_WOOTZ_WALLET_SIGN_MESSAGE_UNLOCK_FIRST);
    LOG(ERROR) << "JANGID KeyringService: Error - " << ret.error_message;
    return ret;
  }

  auto address = account_id->address;
  LOG(ERROR) << "JANGID KeyringService: Signing for address: " << address;
  ...
  if (signature.empty()) {
    ret.signature = std::nullopt;
    ret.error_message =
        l10n_util::GetStringFUTF8(IDS_WOOTZ_WALLET_SIGN_MESSAGE_INVALID_ADDRESS,
  ...
  LOG(ERROR) << "JANGID KeyringService: Signature generated successfully";
  LOG(ERROR) << "JANGID KeyringService: Signature length: " << ret.signature->size();
  LOG(ERROR) << "JANGID KeyringService: Signature (hex): " << ToHex(*ret.signature);
  return ret;
}
...
void KeyringService::ResetAutoLockTimer() {
  if (auto_lock_timer_->IsRunning()) {
    ...
    size_t auto_lock_minutes =
        (size_t)profile_prefs_->GetInteger(kWootzWalletAutoLockMinutes);
...
void KeyringService::CreateKeyringInternal(mojom::KeyringId keyring_id,
  ...
  if (keyring_id == mojom::kDefaultKeyringId) {
    ...
    bitcoin_import_keyrings_[mojom::KeyringId::kBitcoinImport] =
        std::make_unique<BitcoinImportKeyring>(false);
  } else if (keyring_id == mojom::KeyringId::kBitcoinImportTestnet) {
    bitcoin_import_keyrings_[mojom::KeyringId::kBitcoinImportTestnet] =
...
void KeyringService::GetAutoLockMinutes(GetAutoLockMinutesCallback callback) {
  std::move(callback).Run(
      profile_prefs_->GetInteger(kWootzWalletAutoLockMinutes));
}
...
void KeyringService::SetAutoLockMinutes(int32_t minutes,
  ...
  if (minutes != old_auto_lock_minutes) {
    profile_prefs_->SetInteger(kWootzWalletAutoLockMinutes, minutes);
  }
  std::move(callback).Run(true);
}
...
mojom::BitcoinAccountInfoPtr KeyringService::GetBitcoinAccountInfo(
  ...
  if (auto* bitcoin_keyring = GetBitcoinHDKeyringById(keyring_id)) {
    for (const auto& derived_account_info :
      ...
      result->next_receive_address = bitcoin_keyring->GetAddress(
          derived_account_info.account_index,
          mojom::BitcoinKeyId(kBitcoinReceiveIndex, next_receive_index));

      result->next_change_address = bitcoin_keyring->GetAddress(
          derived_account_info.account_index,
  ...
  if (auto* bitcoin_keyring = GetBitcoinImportKeyring(keyring_id)) {
    for (const auto& imported_account_info :
      ...
      auto result = mojom::BitcoinAccountInfo::New();
      const auto next_receive_index =
          imported_account_info.bitcoin_next_receive_address_index.value_or(0);
      const auto next_change_index =
          imported_account_info.bitcoin_next_change_address_index.value_or(0);
...
mojom::AccountInfoPtr KeyringService::GetSelectedWalletAccount() {
  ...
  auto unique_key =
      profile_prefs_->GetString(kWootzWalletSelectedWalletAccount);
