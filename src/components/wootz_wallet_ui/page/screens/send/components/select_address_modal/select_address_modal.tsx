...
interface Props {
  onClose: () => void
  selectedNetwork: WootzWallet.NetworkInfo | undefined
  fromAccountId: WootzWallet.AccountId | undefined
  ...
  (props: Props, forwardedRef) => {
    ...
    const searchValueHasValidExtension = lowercaseSearchValue
      ? endsWithAny(supportedUDExtensions, lowercaseSearchValue) ||
        (selectedAsset?.coin === WootzWallet.CoinType.SOL &&
          endsWithAny(supportedSNSExtensions, lowercaseSearchValue)) ||
        (selectedAsset?.coin === WootzWallet.CoinType.ETH &&
    ...
    const accountsByNetwork = React.useMemo(() => {
      ...
      if (fromAccountId.coin === WootzWallet.CoinType.FIL) {
        ...
        const fevmAccounts = accounts.filter(
          (account) => account.accountId.coin === WootzWallet.CoinType.ETH
        )
    ...
    const { data: fevmTranslatedAddresses } = useGetFVMAddressQuery(
      selectedNetwork &&
        selectedNetwork?.coin === WootzWallet.CoinType.FIL &&
        evmAddressesforFVMTranslation.length
        ? {
            coin: selectedNetwork.coin,
            addresses: evmAddressesforFVMTranslation,
            isMainNet: selectedNetwork.chainId === WootzWallet.FILECOIN_MAINNET
          }
        : skipToken
    )
    ...
    const showFilecoinFEVMWarning =
      fromAccountId?.coin === WootzWallet.CoinType.FIL
    ...
    const { data: isBase58 = false } = useGetIsBase58EncodedSolPubkeyQuery(
      fromAccountId?.coin === WootzWallet.CoinType.SOL && trimmedSearchValue
        ? trimmedSearchValue
        : skipToken
    )

    const {
      data: zecAddressValidationResult = WootzWallet
        .ZCashAddressValidationResult.Unknown
    } = useValidateUnifiedAddressQuery(
      fromAccountId?.coin === WootzWallet.CoinType.ZEC && trimmedSearchValue
        ? {
            address: trimmedSearchValue,
            testnet: selectedNetwork?.chainId === WootzWallet.Z_CASH_TESTNET
          }
        : skipToken
    )
    ...
    const addressMessageId: AddressMessageInfoIds | undefined =
      React.useMemo(() => {
        return processAddressOrUrl({
          ...
          ethAddressChecksum,
          isBase58,
          coinType: fromAccountId?.coin ?? WootzWallet.CoinType.ETH,
          token: selectedAsset,
    ...
    const onSelectAccount = React.useCallback(
      async (account: WootzWallet.AccountInfo) => {
        if (
    ...
    return (
      <PopupModal
        onClose={onClose}
        title={getLocale('wootzWalletChooseRecipient')}
        width='560px'
        height='90vh'
        ref={forwardedRef}
      >
          ...
          >
            ...
            >
              <LabelText
                ...
                isBold={true}
              >
                {getLocale('wootzWalletSendTo')}
              </LabelText>
            </Row>
            <AddressInput
              value={searchValue}
              onInput={(e) => setSearchValue(e.value)}
              placeholder={getLocale('wootzWalletAddressOrDomainPlaceholder')}
              type='text'
                ...
                <>
                  ...
                  >
                    <LabelText
                      ...
                      isBold={true}
                    >
                      {getLocale('wootzWalletMyAddresses')}
                    </LabelText>
                  </Row>
              ...
              {isSearchingForDomain && (
                <Row margin='26px 0px 0px 0px'>
                  ...
                  <Text
                    ...
                    textColor='secondary'
                  >
                    {getLocale('wootzWalletSearchingForDomain')}
                  </Text>
                </Row>
              )}
              {filteredAccounts.length === 0 &&
...
function getAddressMessageInfo({
  ...
  coinType: WootzWallet.CoinType | undefined
...
const processEthereumAddress = (
  addressOrUrl: string,
  token: WootzWallet.BlockchainToken | undefined,
  checksumAddress: string
) => {
  if (
    token &&
    (token.chainId === WootzWallet.FILECOIN_ETHEREUM_MAINNET_CHAIN_ID ||
      token.chainId === WootzWallet.FILECOIN_ETHEREUM_TESTNET_CHAIN_ID) &&
...
const processZCashAddress = (
  zecAddressValidationResult: WootzWallet.ZCashAddressValidationResult
) => {
  if (
    zecAddressValidationResult ===
    WootzWallet.ZCashAddressValidationResult.Unknown
  ) {
    return undefined
  }
  if (
    zecAddressValidationResult ===
    WootzWallet.ZCashAddressValidationResult.InvalidUnified
  ) {
    return AddressMessageInfoIds.invalidUnifiedAddressError
  }
  if (
    zecAddressValidationResult !==
    WootzWallet.ZCashAddressValidationResult.Success
  ) {
    return AddressMessageInfoIds.invalidAddressError
  }
  return undefined
...
function processAddressOrUrl({
  ...
  isBase58: boolean
  zecAddressValidationResult: WootzWallet.ZCashAddressValidationResult
  fullTokenList: WootzWallet.BlockchainToken[]
  ...
  switch (coinType) {
    ...
    case WootzWallet.CoinType.BTC: {
      return processBitcoinAddress(
        addressOrUrl,
        token?.chainId === WootzWallet.BITCOIN_TESTNET
      )
    }
