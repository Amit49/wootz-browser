...
// Types
import { WootzWallet, SignDataSteps } from '../../../constants/types'
...
interface Props {
  signMessageData: WootzWallet.SignMessageRequest[]
  showWarning: boolean
}
...
const onClickLearnMore = () => {
  chrome.tabs.create(
    {
      url: 'https://support.wootz.com/hc/en-us/articles/4409513799693'
    },
    () => {
...
export const SignPanel = (props: Props) => {
  ...
  const [selectedQueueData, setSelectedQueueData] =
    React.useState<WootzWallet.SignMessageRequest>(signMessageData[0])
  ...
  return (
    <StyledWrapper>
        ...
        {signMessageQueueInfo.queueLength > 1 && (
          <QueueStepRow>
            <QueueStepText>
              ...
              {getLocale('wootzWalletQueueOf')}{' '}
              {signMessageQueueInfo.queueLength}
            </QueueStepText>
            <QueueStepButton onClick={onQueueNextSignMessage}>
              {signMessageQueueInfo.queueNumber ===
              signMessageQueueInfo.queueLength
                ? getLocale('wootzWalletQueueFirst')
                : getLocale('wootzWalletQueueNext')}
            </QueueStepButton>
          </QueueStepRow>
        )}
      </TopRow>
      ...
      <PanelTitle>{getLocale('wootzWalletSignTransactionTitle')}</PanelTitle>
      {signStep === SignDataSteps.SignRisk && (
        <WarningBox warningType='danger'>
          <WarningTitleRow>
            <WarningIcon />
            <WarningTitle warningType='danger'>
              {getLocale('wootzWalletSignWarningTitle')}
            </WarningTitle>
          </WarningTitleRow>
          <WarningText>{getLocale('wootzWalletSignWarning')}</WarningText>
          <LearnMoreButton onClick={onClickLearnMore}>
            {getLocale('wootzWalletAllowAddNetworkLearnMoreButton')}
          </LearnMoreButton>
        </WarningBox>
      )}
        ...
        <>
          <TabRow>
            <PanelTab
              ...
              text={
                ethSignTypedData
                  ? getLocale('wootzWalletDetails')
                  : getLocale('wootzWalletSignTransactionMessageTitle')
              }
            />
          </TabRow>
            ...
            <WarningBox warningType='warning'>
              <WarningTitleRow>
                ...
                <WarningTitle warningType='warning'>
                  {getLocale('wootzWalletNonAsciiCharactersInMessageWarning')}
                </WarningTitle>
              </WarningTitleRow>
              <LearnMoreButton
              ...
              >
                {renderUnicode
                  ? getLocale('wootzWalletViewDecodedMessage')
                  : getLocale('wootzWalletViewEncodedMessage')}
              </LearnMoreButton>
            </WarningBox>
          )}
      ...
      <SignPanelButtonRow>
        <NavButton
          ...
          text={getLocale('wootzWalletButtonCancel')}
          onSubmit={onCancel}
          disabled={isDisabled}
        />
        <NavButton
          ...
          text={
            signStep === SignDataSteps.SignData
              ? getLocale('wootzWalletSignTransactionButton')
