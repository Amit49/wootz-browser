...
interface GetFVMAddressArg {
  coin: WootzWallet.CoinType | undefined
  isMainNet: boolean
  addresses: string[]
}
...
export const addressEndpoints = ({
  ...
  return {
      ...
      queryFn: async (_arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          ...
          api.jsonRpcService.setEnsOffchainLookupResolveMethod(
            WootzWallet.ResolveMethod.kEnabled
          )
          return {
            data: true
          }
    ...
    getIsBase58EncodedSolPubkey: query<boolean, string>({
      queryFn: async (pubKeyArg, { endpoint }, _extra, baseQuery) => {
        try {
          const { data: api } = baseQuery(undefined)
          const { result } =
            await api.wootzWalletService.isBase58EncodedSolanaPubkey(pubKeyArg)
    ...
    validateUnifiedAddress: query<
      WootzWallet.ZCashAddressValidationResult,
      {
        address: string,
        testnet: boolean
      }
    ...
    getFVMAddress: query<GetFVMAddressResult, GetFVMAddressArg>({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        if (arg.coin !== WootzWallet.CoinType.FIL) {
          // invalid coin type
          return { data: {} }
        }
        try {
          const { wootzWalletService } = baseQuery(undefined).data
          const convertResult = (
            await wootzWalletService.convertFEVMToFVMAddress(
              arg.isMainNet,
              arg.addresses
            )
          ).result
    ...
    generateReceiveAddress: mutation<string, WootzWallet.AccountId>({
      queryFn: async (accountId, { endpoint }, extraOptions, baseQuery) => {
        try {
          const { wootzWalletService } = baseQuery(undefined).data
          const { address, errorMessage } =
            await wootzWalletService.generateReceiveAddress(accountId)
