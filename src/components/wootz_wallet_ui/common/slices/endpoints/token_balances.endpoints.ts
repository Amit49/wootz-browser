...
export const tokenBalancesEndpoints = ({
  ...
  return {
    ...
    >({
      ...
      async onCacheEntryAdded(
        ...
        function onAnkrBalances(
          accountId: WootzWallet.AccountId,
          ankrAssetBalances: WootzWallet.AnkrAssetBalance[]
        ) {
          ...
          try {
            ...
            if (arg.useAnkrBalancesFeature) {
              const { chainIds: ankrSupportedChainIds } =
                await wootzWalletService.getAnkrSupportedChainIds()
            ...
            await eachLimit(
              ...
              async (accountId: WootzWallet.AccountId) => {
                ...
                const ankrAssetBalances: WootzWallet.AnkrAssetBalance[] =
    ...
    getBitcoinBalances: query<BitcoinBalances, WootzWallet.AccountId>({
      queryFn: async (arg, { endpoint }, extraOptions, baseQuery) => {
        try {
          const { data: api } = baseQuery(undefined)
          const { balance, errorMessage } =
...
async function fetchAnkrAccountBalances(
  jsonRpcService: WootzWallet.JsonRpcServiceRemote,
  accountId: WootzWallet.AccountId,
  chainIds: string[]
) {
  ...
  if (error !== WootzWallet.ProviderError.kSuccess || errorMessage) {
    console.log(
...
async function fetchAccountCurrentNativeBalance({
  ...
  jsonRpcService: WootzWallet.JsonRpcServiceRemote
  bitcoinWalletService: WootzWallet.BitcoinWalletServiceRemote
  zcashWalletService: WootzWallet.ZCashWalletServiceRemote
}): Promise<string> {
  // LOCALHOST
  if (
    token.chainId === WootzWallet.LOCALHOST_CHAIN_ID &&
    accountId.coin !== WootzWallet.CoinType.SOL
  ) {
  ...
  switch (accountId.coin) {
    case WootzWallet.CoinType.SOL: {
      ...
      if (token.chainId === WootzWallet.LOCALHOST_CHAIN_ID && error !== 0) {
        return Amount.zero().format()
      }
    ...
    case WootzWallet.CoinType.ETH: {
      ...
      if (error && errorMessage) {
        throw new Error(
          `getBalance (${
            accountId.coin === WootzWallet.CoinType.FIL ? 'FIL' : 'ETH'
...
async function fetchAccountTokenBalanceRegistryForChainId({
  ...
  if (arg.coin === CoinTypes.ETH) {
    ...
    if (supportedChainIds.includes(arg.chainId)) {
      ...
      if (result.error !== WootzWallet.ProviderError.kSuccess) {
        throw new Error(
          'Error calling ' +
  ...
  await eachLimit(
    nonNativeTokens,
    10,
    async (token: WootzWallet.BlockchainToken) => {
